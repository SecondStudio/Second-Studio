...
// just a copy of shape tail fornow, but smaller radius

/// <summary>
/// the actual generation of splines are made here and I have no idea how
/// </summary>
public class HairSplineTrail : MonoBehaviour
{
    // how often does the script track object's position
    public float seconds = 0.03f;
    public float trackLength = 0.01f;
    ...
    // controls radius and Hair of the mesh
    private float m_Radius = 0.2f; // determines radius
    private int m_RadialSegmentCount; // determines Hair
    ...
    public void BuildMeshTrail(Vector3 pos, Vector3 up, Vector3 forward, Vector3 right , bool Linear)
    {
        ...
        if (Linear && spline.knots.Count >=6) spline.knots.RemoveRange(5, spline.knots.Count - 6);
            ...
            for (int i = begin; i < end; ++i)
            {
                if (i != 0 && beginKnot >= 3)
                {
                    ...
                    if (i * (m_RadialSegmentCount + 1) >= maxPoint)
                        BuildNewHair(position, n, bn, t, r, meshBuilder.Vertices.Count > 0); // add new vertices
                    else
                        BuildExistingHair(position, n, bn, t, r, i); // update vertices and normals*/
                }
            }
    ...
    // Adds new vertices
    protected void BuildNewHair(Vector3 centre, Vector3 normal, Vector3 binormal, Vector3 tangent, float radius, bool buildTriangles)
    {
    ...
    protected void BuildExistingHair(Vector3 centre, Vector3 normal, Vector3 binormal, Vector3 tangent, float radius, int ringIndex)
    {
        float angleInc = (Mathf.PI * 2.0f) / m_RadialSegmentCount;
    ...
    private void addCursorTransform()
    {
        ...
        knotRadius.Add(cursorTransform.GetComponent<ProcHair>().radius);
    ...
    public void Init(GameObject Cursor)
    {
        if (Cursor == null)
        {
            Debug.Log("GameObject 'Hair' is not assigned!");
            Application.Quit();
        }
        ...
        ProcHair ps = Cursor.GetComponent<ProcHair>();
